...
3. 微信小程序：不建立 BLE GATT 连接，只持续扫描 BLE 广播，解析 ESP32-S3 广播中的温湿度数据并显示。
...
本设计取消了原先 ESP32-S3 作为 GATT 服务端、微信小程序连接读特征值的方式，改为无连接 BLE 广播。这样可以减少连接状态管理，微信小程序只需要扫描广播即可得到最新温湿度数据。

## 2. 硬件连接

### 2.1 DHT11 与 FRDM-MCXN947
...
微信小程序不再连接 BLE GATT 服务，而是调用微信 BLE API 持续扫描广播。核心处理流程为：
...
1. 使用 BLE 广播而不是 GATT 连接：小程序只需要实时显示最新温湿度，广播方式无需连接、无需服务发现，状态更简单。
...
以下仅附 FRDM-MCXN947 与 ESP32-S3 两部分核心源代码。

### 9.1 FRDM-MCXN947 Zephyr 主程序：`dht11_thermohygrometer/src/main.c`
...
### 9.2 FRDM-MCXN947 Devicetree Overlay：`dht11_thermohygrometer/app.overlay`
...
### 9.3 ESP32-S3 ESP-IDF 主程序：`dht11_ble/main/main.c`
...
### 9.4 ESP32-S3 Kconfig：`dht11_ble/main/Kconfig.projbuild`
